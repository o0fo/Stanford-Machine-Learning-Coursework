...
Theta1_R = [zeros(size(Theta1), 1) Theta1(:, 2:end)];
Theta2_R = [zeros(size(Theta2), 1) Theta2(:, 2:end)];

reg_term = (lambda/(2*m)) * (sum(sum(Theta1_R .^ 2, 2)) + sum(sum(Theta2_R .^ 2, 2)));
J = J + reg_term;
...
for i = 1:m
	...
	d_3 = a_3 - Y(i,:);		%(1x10)
	d_2 = (d_3*Theta2)(:, 2:end) .* sigmoidGradient(a_1 * Theta1');		%1x25
